...
class Mako(VODFetcher):
    ...
    def _update_base_categories(self, base_object):
        ...
        raw_data = req.json()
        base_object.add_sub_objects([CatalogNode(catalog_manager=self.catalog_manager,
                                                 obj_id=x['guid'],
                                                 title=x['title'],
    ...
    def get_video_links_from_video_data(self, video_data):
        ...
        parse_url = urlparse(video_data.url)
        params = dict((x.split('=') for x in parse_url.query.split('&')))
        params['type'] = 'service'
        params['device'] = 'desktop'
        new_url = parse_url.scheme + '://' + parse_url.hostname + parse_url.path
        headers = {
        ...
        for x in raw_res['data']:
            if len(x['guid']) > 0:
                ...
                if self.catalog_manager.is_node(x['guid']):
                    ...
                    res.append(CatalogNode(catalog_manager=self.catalog_manager,
                                           obj_id=x['guid'],
                                           title=x['label'],
                                           url=urljoin(self.base_url, x['url']),
                                           image_link=x['picB'],
    ...
    def _get_show_from_show_object(self, show_object):
        ...
        for x in show_raw_data['root']['programData']['seasons']:
            season = CatalogNode(catalog_manager=self.catalog_manager,
                                 obj_id=x['id'],
                                 title=x['name'],
                                 url=urljoin(self.base_url, x['url']),
...
class Bip(Mako):
    ...
    def _update_base_categories(self, base_object):
        ...
        for tree in show_trees:
            ...
            obj = CatalogNode(catalog_manager=self.catalog_manager,
                              obj_id=url,
                              title=title,
    ...
    def _get_show_from_show_object(self, show_object):
        ...
        for tree in show_trees:
            ...
            episode_obj = CatalogNode(catalog_manager=self.catalog_manager,
                                      obj_id=url,
                                      title=title,
